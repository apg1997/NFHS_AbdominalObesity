...
pacman::p_load(tidyverse,lubridate,rio,janitor,here,gtsummary,survey,haven,labelled,sjlabelled,performance,jtools,sessioninfo)
```
...
export(women_state_wc_df,here("Output","women_state_obesity_prev_wc.xlsx"))
# save all objects with women_ prefix except "women_original"and "women_model_adj_wc"
save(list = setdiff(ls(pattern = "^women_"), c("women_original","women_model_adj_wc")), file = here("Output", "women_output.Rdata"))

# save all objects with women_ prefix
...
```{r session info}
sessioninfo::session_info(pkgs = c("loaded", "attached")[1], to_file = here("Session info","session_info_women.txt"))
```