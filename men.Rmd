...
prev_bmi_men <- tbl_svysummary(NFHS5_design, by = obese_bmi, 
                               include = c(age_category,residence,education,religion,wealth,caste,meat),
                               statistic = list(all_categorical() ~ "{p}% ({n})"),
                               missing_text = "NA",
                               percent = "row") %>% 
...
model_adj_wc_men <- svyglm(obese_wtcircum ~ age_category+residence+education+religion+wealth+caste+meat, 
                           design=NFHS5_design, data = men, family = binomial())

# model_adj_wc_men %>% 
#   model_performance()
# 
# summ(model_adj_wc_men, vifs = T)

model_uv_wc_men_est <- model_uv_wc_men %>% 
  tbl_regression(exponentiate = T)

model_adj_wc_men_est <- model_adj_wc_men %>% 
  tbl_regression(exponentiate = T)

model_uv_wc_men_est
model_adj_wc_men_est
```

```{r prevalence by state-district}
# obesity prevalence disaggregated by state
state_wc_men <- tbl_svysummary(NFHS5_design, by = obese_wtcircum, 
                               include = c(state),
                               statistic = list(all_categorical() ~ "{p}% ({n})"),
                               missing_text = "NA",
                               percent = "row") %>% 
...
district_wc_men <- tbl_svysummary(NFHS5_design, by = obese_wtcircum, 
...
final_district_wc_men <- merge(district,district_details, by = "district_id") %>% 
  mutate(state = str_to_title(state),
         district = str_to_title(district))
```

```{r export}
export(final_district,here("Output","men_dist_obesity_prev_wc.xlsx"))
```
