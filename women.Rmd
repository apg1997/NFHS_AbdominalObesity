...
ddistrict_est <- svyby(~obese_wtcircum, ~district_id, NFHS5_design, svymean, keep.names = FALSE, na.rm = TRUE, vartype = c("se", "ci"), multicore = TRUE) %>%
  select(!contains("Non-")) %>% 
  rename(prev_obese = obese_wtcircumObese,
         se_obese = se.obese_wtcircumObese,
         ci_l_obese = ci_l.obese_wtcircumObese,
...
write.xlsx(final_district,"women_dist_obesity_prev.xlsx")
```
