...
import_women <- haven::read_dta(here("Data_raw","IAIR7EFL","IAIR7EFL.DTA"), 
...
# haven and sjlabelled labelling function to be used later. Integrate with the women code above.
...
transformations <- list(
  ...
  "Step 7 - Filtering NA in waist circumference" = 
...
district_est <- svyby(~obese_wtcircum, ~state+district, NFHS5_design, svymean, keep.names = FALSE, na.rm = TRUE, vartype = c("se", "ci")) %>%
  select(!contains("Non-")) %>% 
  rename(prev_obese = obese_wtcircumObese,
         se_obese = se.obese_wtcircumObese,
         ci_l_obese = ci_l.obese_wtcircumObese,
...
district_count <- svyby(~obese_wtcircum, ~state+district, NFHS5_design, svytotal, keep.names = FALSE, na.rm = TRUE) %>% 
  select(!starts_with("se.")) %>% 
  mutate(n = round(rowSums(select(., `obese_wtcircumNon-obese`,`obese_wtcircumObese`), na.rm = TRUE),0)) %>% 
  select(district,n)

district <- merge(district_est, district_count, by = "district")
```
