...
p_load(base, broom, dplyr, geojsonio, ggmap, ggplot2, ggthemes,
  ...
  pacman, RColorBrewer, sf, tidyterra, tmap, tmaptools, openxlsx, wesanderson,rio,janitor,here,ggpattern)
...
menplot_districts <- ggplot() + 
  ...
  geom_sf(data = underlay_map,aes(color ="No Data",pattern = "stripe")) +
    scale_color_manual("",values = c("No Data" = "black"),guide = guide_legend(override.aes = list(linetype = "solid", shape = NA))) +  geom_sf(data = merged_data_with_shape_sf, aes(fill = prev_obese_men)) +
...
womenplot_districts <- ggplot() + 
  ...
  scale_color_manual("",values = c("No Data" = "black"),guide = guide_legend(override.aes = list(linetype = "solid", shape = NA))) +
