...
p_load(tidyverse, rio, janitor, here, geojsonio, ggmap, ggthemes, maptiles, wesanderson, RColorBrewer, sf, tidyterra, svglite)
```
...
# Create a custom color palette using wes Anderson colors

state_menplot_cat <- ggplot() + 
  geom_sf(data = state_merged_data_with_shape_state_sf, aes(fill = cat_obese_men)) +
  scale_fill_manual(values = met.brewer("Cassatt1",n = 5, direction =-1, "continuous")) +
  theme_void() +  
  labs(title = "Prevalence of Abdominal Obesity in Men by state", fill = "Prevalence in % age")
